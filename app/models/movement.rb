class Movement < ApplicationRecord
  ...
  def self.generate_monthly_payments
    ...
    users.each do |user|
      if user[:paydate] && Date.today >= user[:paydate] - 3.days && user.movements.where(ambassador: true) && user.movements.where(ambassador: true).last && user.movements.where(ambassador: true).last[:status] != "No pagado"
